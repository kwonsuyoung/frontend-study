...
const p1 = new Promise((resolve, reject) => {
    if (val % 2) { //홀수
        resolve('홀수'); //then 메소드에 전달된 콜백함수가 호출, 아규먼트 값이 넘어간다.
    } else { //짝수
        reject('짝수'); //catch메소드에 전달된 콜백함수가 호출, 아규먼트 값이 넘어간다.
    }
}); //p1에는 Promise 객체 주소값 저장됨

p1.then(item => console.log('then-item:', item)) 
